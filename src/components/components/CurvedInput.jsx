...
const CurvedInput = ({
  ...
  useLayoutEffect(() => {
    ...
    if (caretLen - next > layout.winLen - 2)
      next = caretLen - layout.winLen + 2;
    if (caretLen - next < 0) next = caretLen;
    if (totalLen - next < layout.winLen)
      next = Math.max(0, totalLen - layout.winLen);
    ...
    content = (
      ...
      >
        ...
        {showButton && (
          ...
          >
            <path
              className="curved-input__button-bg"
              d={buttonPath}
              fill={accentColor}
            />
