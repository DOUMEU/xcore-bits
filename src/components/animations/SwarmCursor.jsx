...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    let activeCount = Math.max(
      1,
      Math.min(MAX, Math.round(propsRef.current.count)),
    );
    ...
    const frame = (now) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const orbit =
          band * (0.34 + 1.35 * Math.max(0, Math.min(1, orbitDrift + 0.5)));

        const radial = Math.max(
          -1,
          Math.min(1, (dist - orbit) / (band * 0.85)),
        );
        ...
        if (flowMix > 0.001) {
          ...
          const bt = t * 0.22;
          const coarseX =
            (noise3(perm, bx, by + eps, bt) - noise3(perm, bx, by - eps, bt)) /
            (2 * eps);
          const coarseY =
            -(noise3(perm, bx + eps, by, bt) - noise3(perm, bx - eps, by, bt)) /
            (2 * eps);
          ...
          const ft = t * 0.55;
          const fineX =
            (noise3(perm, fx, fy + eps, ft) - noise3(perm, fx, fy - eps, ft)) /
            (2 * eps);
          const fineY =
            -(noise3(perm, fx + eps, fy, ft) - noise3(perm, fx - eps, fy, ft)) /
            (2 * eps);
  ...
  return (
    ...
    >
      {children ? (
        <div className="swarm-cursor__content">{children}</div>
      ) : null}
    </div>
  );
};
